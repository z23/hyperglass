...
Supply environment variables in the shell that starts hyperglass. The sample
systemd units also load `/etc/hyperglass/hyperglass.env` using `EnvironmentFile`.
Direct launches do not automatically load that file; direct Compose commands can
load it with `--env-file /etc/hyperglass/hyperglass.env`.
...
| `HYPERGLASS_RATE_LIMIT`        | number  | `10`              | Maximum number of `/api/query` requests allowed per client, per period. Also settable as `rate_limit.limit` in `config.yaml`. |
| `HYPERGLASS_RATE_LIMIT_PERIOD` | string  | `minute`          | Rate-limit window: `second`, `minute`, `hour`, or `day`.                                                           |
| `HYPERGLASS_RATE_LIMIT_ENABLE` | boolean | `true`            | Enable per-client rate limiting on the unauthenticated `/api/query` endpoint.                                      |
| `FORWARDED_ALLOW_IPS` | string | `127.0.0.1` (Uvicorn); `::1` in the native systemd sample | Exact proxy peers trusted to supply client addresses. Use `::1` for the native IPv6-loopback samples; configure the container-visible peer for Docker. Empty trusts no proxy. See [Reverse Proxy](reverse-proxy.mdx). |
